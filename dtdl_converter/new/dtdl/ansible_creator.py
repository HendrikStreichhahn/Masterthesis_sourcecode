...
class ansibleInfoDTDLInstance:
    ...
    def getBashCommandTasks(self,
        ...
        for i, command in enumerate(self.commands):
            ...
            elif command.type == "once_no_wait" or command.type == string_helper.UNDEFINED_STRING :
                result = string_helper.addLine(result,
                    string_helper.char_n_times(indentationCount+1, indentationChar) +
                    "ansible.builtin.shell: \"" + command.command + args + "\"")
    ...
    def getTasksString(
            ...
            indentationChar: str = "  ",
            indentationCount: int = 0):
        folder, _ = os.path.split(playbookpath)
        if not self.telegraf_config_file_added:
            self.add_telegraf_conf_filetransmissions(playbookpath, telegraf_config_path)
    ...
    def readInstanceFromJson(self, jsonDef: json, base_path: str):
        ...
        print(f"read instance {self.instanceID}")
        ...
        if "commands" in jsonDef:
            for content in jsonDef["commands"]:
                ...
                #     wait = content["wait"]
                # else:
                #     wait= string_helper.UNDEFINED_STRING
                if "type" in content:
                    type = content["type"]
                else:
    ...
    def add_telegraf_conf_filetransmissions(
        ...
        # inpPath = self.get_telegraf_config_path(telegraf_config_folder)
        # absPathB = string_helper.get_abs_path(inpPath, base_path)
        absPath = string_helper.get_abs_path(
                        telegraf_config_folder,
                        base_path)
        self.fileTransmissionCommands.append(
            fileTransmissionCommand(
                self.get_telegraf_config_path(absPath),
...
class ansiblePlaybookDTDL:
    ...
    def savePlaybook(self, path: str, telegraf_config_path: str, playbookpath: str, servicesPath: str):
        result = "---"
        for instance in self.instances:
            instance.generateCommandAsServiceFiles(servicesPath)
            result = string_helper.addLine(result, instance.getTasksString(telegraf_config_path, playbookpath))
